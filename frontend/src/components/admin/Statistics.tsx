...
export default function Statistics() {
  ...
  const loadData = () => {
    ...
    // Calcul des nouveaux clients (derniers 30 jours)
    const newCustomers = users.filter(u => {
      const createdAt = new Date(parseInt(u.id.substring(1)));
      return createdAt > thirtyDaysAgo;
    }).length;
    
    // Calcul des clients fidèles (plus d'une commande)
    ...
    const lastMonthOrders = orders.filter(o => 
      new Date(o.timestamp) > thirtyDaysAgo && 
      o.status !== 'cancelled'
    );
    ...
    const previousMonthSales = previousMonthOrders.reduce((sum, o) => sum + o.total, 0);
    const salesPercentage = previousMonthSales === 0 ? 100 :
  ...
  // Fonctions de génération des données pour les graphiques
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-sm text-gray-500">Plus d'une commande</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats.topSellingItems[0]?.value || 0}
            </div>
            <p className="text-sm text-gray-500">
              {stats.topSellingItems[0]?.name || 'Aucune vente'}
            </p>
          </CardContent>
        </Card>
      </div>
